...
- A case with a `if(__vmMode===VM_SOLO){ // SOLO copy, kept as it was` block is one where the solo
  machine has no partner and substitutes 0 or a no-op for a partner read. 37 of these remain. Op 16 (BUD) was
  reverted in #231c on a false premise, reopened in #231d, and CLOSED by #231e's pre-registered rule. The only other machine-dependence is context (#231b): op 9's depositor signature and solo strength, and
